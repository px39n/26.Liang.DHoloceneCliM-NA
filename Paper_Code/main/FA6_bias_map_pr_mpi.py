"""FA6: Mean bias + percentile difference maps for precipitation (MPI-ESM).

Appendix version of F6 for MPI-ESM-CR.
"""
...
OUT = RESULTS_MAIN / "FA6_bias_map_pr_mpi.png"
GRID_DIR = DATA_DIR / "interim" / "mpi-esm-cr" / "grid_cal"
...
def main():
    print(f"FA6: Loading MPI-ESM gridded test data for {VAR} ...", flush=True)
    ...
    for row_title, (left, right, _) in zip(
        ...
        for side, data in [("PCR−Obs", left), ("ESM−Obs", right)]:
            sm = np.nanmean(data)
            ss = np.nanstd(data)
            print(f"  {row_title} {side}: {sm:+.4f} ± {ss:.4f} {UNIT}", flush=True)
    ...
    for row_i, (left, right, title) in enumerate(panels):
        for col_i, (data, label) in enumerate(
            ...
            ax.text(0.03, 0.07, f"{spatial_mean:+.3f} ± {spatial_std:.3f}",
    ...
    fig.suptitle(f"MPI-ESM 1.2 CR — Precipitation Validation ({UNIT})",
                 fontsize=15, fontweight="bold", y=1.01)
    fig.savefig(OUT, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {OUT}", flush=True)
    sync_output(OUT, is_supplementary=True)
