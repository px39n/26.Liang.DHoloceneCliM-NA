"""FA1: GHCN station coverage map for MPI-ESM-CR (Appendix).

Same layout as F1 but uses MPI-ESM-CR split_calibration.pkl (cal 1875-1949).
Output: FA1_station_coverage_mpi.png
"""
import sys
...
OUT = RESULTS_MAIN / "FA1_station_coverage_mpi.png"
...
def _plot_panel(ax, meta, var_label, panel_label):
    ...
    ax.set_title(f"{var_label} GHCN-m stations (MPI-ESM-CR, 1875\u20131949)",
...
def main():
    ...
    plt.close(fig)
    sync_output(OUT, is_supplementary=False)
    print(f"Done: {OUT}")
